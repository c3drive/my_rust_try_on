...
#[derive(Template)]
#[template(path = "stopwatch.html")]
struct StopwatchTemplate {
    entries: Vec<TodoEntry>,
}
...
#[get("/stopwatch")]
async fn stopwatch(db: web::Data<Pool<SqliteConnectionManager>>) -> Result<HttpResponse, MyError> {
    let conn = db.get()?;
    let mut statement = conn.prepare("SELECT id, text FROM todo")?;
    let rows = statement.query_map(params![], |row| {
        let id = row.get(0)?;
    ...
    let html = StopwatchTemplate { entries };
    let response_body = html.render()?;
    Ok(HttpResponse::Ok()
        .content_type("text/html")
        .body(response_body))
